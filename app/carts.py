...
def shopping_cart():
    ...
    if request.method == 'POST':
        ...
        if ProductCart.update_incart_quantity(cart_key, product_key, seller_key, new_quantity):
            flash('Cart updated successfully', 'success')
        else:
            flash('Error updating cart', 'error')
        return redirect(url_for('.shopping_cart'))

    # Retrieve cart products along with subtotal
    cart_items = Cart.get_incart_products_by_userkey(c_userkey=user_key)
...
def checkout():
    ...
    has_balance, message = check_balance(user_key, total_cost)
    if has_balance:
